...
def main():
    ap = argparse.ArgumentParser(description="Generate predictions with trained model.")
    ...
    ap.add_argument("--artifacts_dir", default="artifacts_logreg", help="Directory containing model.pkl and metadata.json.")
    args = ap.parse_args()
    ...
    # --- Console summary ---
    avg_conf = np.mean(proba) * 100
    success_rate = np.mean(preds) * 100
    print("\nPrediction Summary")
    print(f"Average Confidence: {avg_conf:.2f}%")
    print(f"Predicted Success Rate: {success_rate:.1f}% ({np.sum(preds)}/{len(preds)} rows)")
